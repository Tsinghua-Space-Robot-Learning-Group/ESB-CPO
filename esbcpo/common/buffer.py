...
class Buffer:
    def __init__(self,
        ...
        self.discounted_cost_buf = np.zeros(size, dtype=np.float32)
    ...
    def store(self, obs, act, rew, val, logp, cost=0., cost_val=0., safety_state=0.):
        """
    ...
    def finish_path(self, last_val=0, last_cost_val=0, penalty_param=0):
        ...
        # cumsum_cost = discount_cumsum(costs, self.gamma)[:-1][::-1] / (np.array(range(len(costs - 1))) + 1)
        ...
        self.discounted_cost_buf[path_slice] = discount_cumsum(costs, self.gamma)[:-1]
        ...
        self.start_ptr_list.append(self.ptr)
    ...
    def get(self):
        ...
        data = dict(
            ...
            rew=self.rew_buf,
            cost=self.cost_buf,
            cost_val = self.cost_val_buf,
            discounted_ret=self.discounted_ret_buf,
            discounted_cost=self.discounted_cost_buf,
            cost_adv=self.cost_adv_buf, target_c=self.target_cost_val_buf,
            safety_state=self.safety_state_buf,
        )

        data = {k: torch.as_tensor(v, dtype=torch.float32) for k, v in
                data.items()}

        data.update({"start_ptr_list": self.start_ptr_list})
        self.start_ptr_list = []
